...
export class Spell {
  constructor(player, id, name) {
    ...
    if (spell.timetoendactive)
      this.timetoend = parseInt(spell.timetoend) * 1000;
    if (spell.timetostartactive)
      this.timetostart = parseInt(spell.timetostart) * 1000;
    ...
    if (spell.switchtimeactive)
      this.switchtime = parseFloat(spell.switchtime) * 1000;
    ...
    if (spell.swingtimeractive)
      this.swingtimer = parseFloat(spell.swingtimer) * 1000;
    if (spell.priority) this.priority = parseInt(spell.priority);
    if (spell.expriority) this.expriority = parseInt(spell.expriority);
    if (spell.switchechoesactive)
      this.switchechoestime = parseFloat(spell.switchechoestime) * 1000;
    if (spell.switchechoesactive)
      this.switchechoesrage = parseInt(spell.switchechoesrage);
    if (spell.switchechoesactive)
      this.switchechoesactive = spell.switchechoesactive;
    if (spell.switchoractive)
      this.switchortime = parseFloat(spell.switchortime) * 1000;
  ...
  step(a) {
    if (this.timer <= a) {
      ...
      /* start-log */ if (this.player.logging)
        this.player.log(`${this.name} off cooldown`); /* end-log */
    } else {
      this.timer -= a;
    }
    return this.timer;
...
export class Whirlwind extends Spell {
  ...
  dmg() {
    ...
    dmg +=
      (this.player.stats.ap / 14) * this.player.mh.normSpeed +
  ...
  use() {
    if (!this.player.isValidStance("zerk")) {
      ...
      if (this.player.switchdelay && this.player.stance == "glad")
        stance =
          this.player.basestance == "glad"
            ? this.player.spells.unstoppablemight.secondarystance
            : this.player.basestance;
  ...
  canUse() {
    return (
      ...
      !this.player.timer &&
      this.cost <= this.player.rage &&
      (this.player.isValidStance("zerk") ||
        this.player.talents.rageretained >= this.cost) &&
      (!this.maxrage ||
        this.player.isValidStance("zerk") ||
        this.player.rage <= this.maxrage) &&
  ...
  use(delayedheroic) {
    if (
      ...
      if (this.player.switchdelay && this.player.stance == "glad")
        ...
        this.player.spells.unstoppablemight &&
        this.player.spells.unstoppablemight.secondarystance == "battle"
      )
        stance = "battle";
      this.player.switch(stance);
    }
    ...
    if (delayedheroic && delayedheroic.exmacro) {
      if (delayedheroic.canUse()) {
        ...
        delayedheroic instanceof Cleave &&
        delayedheroic.backupheroic &&
        delayedheroic.backupheroic.canUse()
      ) {
    ...
    this.totalusedrage +=
      this.usedrage -
      (this.player.auras.suddendeath && this.player.auras.suddendeath.timer
        ? 10
        : 0);
    this.timer = 1 - (step % 1);
    this.maxdelay = rng(this.player.reactionmin, this.player.reactionmax);
...
export class Bloodrage extends Spell {
  constructor(player, id) {
    ...
    if (player.basestance == "glad" && player.gladbloodrage)
      this.cooldown -= 30;
  }
  use() {
    this.timer = this.cooldown * 1000;
    ...
    this.maxdelay = rng(this.player.reactionmin, this.player.reactionmax);
    if (
      this.player.auras.consumedrage &&
      oldRage < 60 &&
      this.player.rage >= 60
    )
...
export class HeroicStrike extends Spell {
  constructor(player, id) {
    ...
    this.useonly = true;
    this.unqueuetimer =
      300 + rng(this.player.reactionmin, this.player.reactionmax);
  }
  use() {
...
export class ThunderClap extends Spell {
  ...
  dmg() {
    ...
    dmg =
      this.value1 +
      (this.player.mode == "sod" ? ~~(this.player.stats.ap * 0.05) : 0);
    if (this.player.furiousthunder) dmg *= 2;
    return dmg * this.player.stats.dmgmod;
  }
  use() {
...
export class QuickStrike extends Spell {
  ...
  dmg() {
    let dmg,
      mod = 1;
    dmg =
      ~~rng(this.player.stats.ap * 0.25, this.player.stats.ap * 0.35) +
      this.player.stats.moddmgdone;
    if (this.player.heroicbonus) mod = 1.25;
    return dmg * this.player.stats.dmgmod * mod;
  }
...
export class RagePotion extends Spell {
  ...
  use() {
    ...
    this.player.rage = Math.min(
      this.player.rage + ~~rng(this.value1, this.value2),
      100,
    );
    this.maxdelay = rng(this.player.reactionmin, this.player.reactionmax);
    if (
  ...
  canUse() {
    return (
      this.timer == 0 && this.player.rage < this.minrage && step >= this.usestep
    );
  }
}
...
export class Slam extends Spell {
  constructor(player, id) {
    ...
    this.casttime = player.precisetiming
      ? 0
      : 1500 - player.talents.impslam * 100;
  ...
  dmg(weapon) {
    ...
    dmg =
      this.value1 +
      rng(weapon.mindmg + weapon.bonusdmg, weapon.maxdmg + weapon.bonusdmg);
    dmg +=
      (this.player.stats.ap / 14) * weapon.speed + this.player.stats.moddmgdone;
    if (this.player.heroicbonus) mod = 1.25;
    return dmg * this.player.stats.dmgmod * mod;
  }
  use() {
    ...
    this.timer = this.cooldown * 1000;
    /* start-log */ if (this.player.logging)
      this.player.log(`${this.name} done casting`); /* end-log */
  }
  canUse() {
    return (
      !this.timer &&
      !this.player.timer &&
      this.player.mh.timer >= this.mhthreshold &&
      (this.player.freeslam || this.cost <= this.player.rage) &&
...
export class Fireball extends Spell {
  ...
  use() {
    ...
    this.idmg += procdmg;
    /* start-log */ if (this.player.logging)
      this.player.log(`Fireball hit for ${procdmg}`); /* end-log */
  }
  canUse() {
    return !this.timer && step >= this.usestep;
  }
...
export class GunAxe extends Spell {
  ...
  use() {
    ...
    this.idmg += procdmg;
    /* start-log */ if (this.player.logging)
      this.player.log(`Gun Axe hit for ${procdmg}`); /* end-log */
  }
  canUse() {
    return !this.timer && step >= this.usestep;
  }
...
export class BlademasterFury extends Spell {
  ...
  use() {
    ...
    if (this.player.spells.whirlwind) {
      ...
      /* start-log */ if (this.player.logging)
        this.player.log(
          `${this.player.spells.whirlwind.name} off cooldown`,
        ); /* end-log */
    }
  }
  canUse() {
...
export class ShieldSlam extends Spell {
  ...
  dmg() {
    ...
    let ap =
      this.player.stats.ap +
      (this.player.auras.defendersresolve &&
      !this.player.auras.defendersresolve.timer
        ? 4 * this.player.stats.defense
        : 0);
    dmg =
      rng(this.value1, this.value2) +
      this.player.stats.block * 2 +
      ~~(ap * 0.15);
    return dmg * this.player.stats.dmgmod * this.player.mainspelldmg;
  }
  use() {
  ...
  canUse() {
    return (
      ...
      !this.timer &&
      !this.player.timer &&
      (this.player.freeshieldslam || this.cost <= this.player.rage) &&
...
export class TheMoltenCore extends Spell {
  ...
  use() {
    ...
    this.idmg += procdmg;
    /* start-log */ if (this.player.logging)
      this.player.log(`The Molten Core hit for ${procdmg}`); /* end-log */
  }
}
...
export class UnstoppableMight extends Spell {
  ...
  canUse() {
    ...
    if (
      this.player.auras.battleforecast &&
      this.switchtimeactive &&
      this.player.stance == this.player.basestance &&
      this.player.auras[this.secondarystance + "forecast"].timer - step <=
        this.switchtime &&
      this.player.rage <= this.switchrage
    ) {
    ...
    if (
      ...
      this.player.auras[this.player.basestance + "forecast"].timer - step <=
        this.switchtime &&
      this.player.rage <= this.switchrage
    ) {
    ...
    if (
      ...
      (this.player.auras[this.secondarystance + "forecast"].timer - step <=
        this.switchortime ||
        this.player.rage <= this.switchorrage)
    ) {
      this.switchto = this.secondarystance;
      return true;
    }
    ...
    if (
      this.switchechoesactive &&
      this.player.stance == this.player.basestance &&
      this.player.auras["echoes" + this.secondarystance].timer - step <=
        this.switchechoestime &&
      this.player.rage <= this.switchechoesrage
    ) {
      this.switchto = this.secondarystance;
      return true;
    }
    ...
    if (
      this.switchechoesactive &&
      this.player.stance == this.secondarystance &&
      this.player.auras["echoes" + this.player.basestance].timer - step <=
        this.switchechoestime &&
      this.player.rage <= this.switchechoesrage
    ) {
      this.switchto = this.player.basestance;
...
export class Aura {
  constructor(player, id, name) {
    ...
    if (spell.echargeblockactive)
      this.echargeblock = parseInt(spell.echargeblock);
  ...
  use() {
    ...
    this.maxdelay = rng(this.player.reactionmin, this.player.reactionmax);
    /* start-log */ if (this.player.logging)
      this.player.log(`${this.name} applied`); /* end-log */
  }
  step() {
    if (step >= this.timer) {
  ...
  prep(duration, itemdelay) {
    ...
    if (typeof this.timetoend !== "undefined") {
      if (this.item && !this.noitemcd) {
        this.usestep = Math.max(
          Math.min(
            duration - this.timetoend,
            duration - itemdelay - this.duration * 1000,
          ),
          0,
        );
...
export class DeepWounds extends Aura {
  ...
  tickdmg(offhand) {
    ...
    if (!offhand) {
      ...
      min =
        this.player.oh.mindmg +
        this.player.oh.bonusdmg +
        this.player.stats.moddmgdone +
        (this.player.stats.ap / 14) * this.player.oh.speed;
      max =
        this.player.oh.maxdmg +
        this.player.oh.bonusdmg +
        this.player.stats.moddmgdone +
        (this.player.stats.ap / 14) * this.player.oh.speed;
    }
    let dmg = (min + max) / 2;
    dmg *=
      (!offhand ? this.player.mh.modifier : this.player.oh.modifier) *
      this.player.stats.dmgmod *
      this.player.talents.deepwounds *
      this.player.bleedmod;
    return dmg;
  }
  step() {
    while (step >= this.nexttick) {
      ...
      /* start-log */ if (this.player.logging)
        this.player.log(
          `${this.name} tick for ${dmg.toFixed(2)}`,
        ); /* end-log */
...
export class OldDeepWounds extends Aura {
  ...
  step() {
    while (step >= this.nexttick) {
      ...
      /* start-log */ if (this.player.logging)
        this.player.log(
          `${this.name} tick for ${(dmg / 4).toFixed(2)}`,
        ); /* end-log */
...
export class BloodFury extends Aura {
  ...
  canUse() {
    return (
      this.firstuse && !this.timer && !this.player.timer && step >= this.usestep
    );
  }
}
...
export class Windfury extends Aura {
  ...
  proc() {
    if (this.stacks < 2) {
      if (step < this.mintime) this.timer = this.mintime;
      else this.step();
      this.stacks = 0;
    } else {
      this.stacks--;
...
export class Swarmguard extends Aura {
  ...
  proc() {
    ...
    this.player.updateArmorReduction();
    /* start-log */ if (this.player.logging)
      this.player.log(`${this.name} proc`); /* end-log */
  }
  step() {
    if (step >= this.timer) {
      this.uptime += this.timer - this.starttimer;
...
export class Gabbar extends Aura {
  ...
  step() {
    if ((step - this.starttimer) % 2000 == 0) {
      ...
      this.player.updateAP();
      /* start-log */ if (this.player.logging)
        this.player.log(`${this.name} tick`); /* end-log */
    }
    if (step >= this.timer) {
      this.uptime += this.timer - this.starttimer;
      this.timer = 0;
...
export class BattleShout extends Aura {
  constructor(player, id) {
    ...
    this.stats.ap = ~~(
      (this.value1 + lvlbonus + (this.player.enhancedbs ? 30 : 0)) *
...
export class Rend extends Aura {
  ...
  step() {
    while (step >= this.nexttick && this.stacks) {
      ...
      /* start-log */ if (this.player.logging)
        this.player.log(
          `${this.name} tick for ${this.tickdmg.toFixed(2)}`,
        ); /* end-log */
      ...
      if (this.player.tasteforblood && this.tfbstep + 6000 <= step) {
        ...
        this.tfbstep = step;
        /* start-log */ if (this.player.logging)
          this.player.log(`Taste of Blood applied`); /* end-log */
      }
    }
  ...
  use() {
    ...
    if (
      !this.player.isValidStance("def", true) &&
      !this.player.isValidStance("battle", true)
    ) {
      let stance = "battle";
      if (this.player.switchdelay && this.player.stance == "glad")
        stance =
    ...
    let dmg =
      basedmg * this.player.stats.dmgmod * this.dmgmod * this.player.bleedmod;
  ...
  canUse() {
    ...
    if (this.stacks) this.uptime += step - this.starttimer;
    this.timer = 0;
    this.stacks = 0;
    this.tfbstep = -6000;
    this.player.updateDmgMod();
  ...
  refresh() {
    ...
    this.stacks = this.value2;
    /* start-log */ if (this.player.logging)
      this.player.log(`${this.name} refreshed`); /* end-log */
  }
}
...
export class WeaponBleed extends Aura {
  ...
  step() {
    while (step >= this.nexttick) {
      ...
      /* start-log */ if (this.player.logging)
        this.player.log(`${this.name} tick for ${this.dmg}`); /* end-log */
...
export class Spicy extends Aura {
  ...
  use() {
    ...
    if (!this.player.attackproc1)
      this.player.attackproc1 = { chance: 500, magicdmg: 7, spicy: true };
    if (!this.player.attackproc2)
      this.player.attackproc2 = { chance: 500, magicdmg: 7, spicy: true };
    /* start-log */ if (this.player.logging)
      this.player.log(`${this.name} applied`); /* end-log */
  }
  step() {
    if (step >= this.timer) {
      ...
      this.player.updateHaste();
      this.player.updateHasteDamage();
      if (this.player.attackproc1 && this.player.attackproc1.spicy)
        delete this.player.attackproc1;
      if (this.player.attackproc2 && this.player.attackproc2.spicy)
        delete this.player.attackproc2;
      /* start-log */ if (this.player.logging)
        this.player.log(`${this.name} removed`); /* end-log */
    }
  }
...
export class CoinFlip extends Aura {
  ...
  use(a, prepull = 0) {
    ...
    if (this.alwaysheads || rng10k() < 5000) {
      ...
      this.player.updateAuras();
      /* start-log */ if (this.player.logging)
        this.player.log(`${this.name} Crit applied`); /* end-log */
    }
  }
  canUse() {
    return this.firstuse && !this.timer && step >= this.usestep;
...
export class VoodooFrenzy extends Aura {
  ...
  use() {
    ...
    this.starttimer = step;
    this.cooldowntimer = step + this.cooldown * 1000;
    if (this.player.stats.str >= this.player.stats.agi)
      this.stats = { str: 35 };

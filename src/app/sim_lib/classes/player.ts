...
export class Player {
  ...
  mainspelldmg: number;
  extraattacks: number;
  batchedextras: number;
  nextswinghs: boolean;
  nextswingcl: boolean;
  freeslam: boolean;
  ragecostbonus: number;
  logging: boolean;
  race: Race;
  aqbooks: boolean;
  reactionmin: number;
  ...
  initStances() {
    ...
    // Might set bonus
    if (
      this.spells.unstoppablemight &&
      this.spells.unstoppablemight.switchstart
    ) {
    ...
    for (let set of sets) {
      ...
      for (let bonus of set.bonus) {
        if (counter >= bonus.count) {
          ...
          if (bonus.stats.heroicbonus)
            this.heroicbonus = bonus.stats.heroicbonus;
          if (bonus.stats.slammainreset)
            this.slammainreset = bonus.stats.slammainreset;
          if (bonus.stats.shieldslamcd)
            this.shieldslamcd = bonus.stats.shieldslamcd;
          if (bonus.stats.gladbloodrage)
            this.gladbloodrage = bonus.stats.gladbloodrage;
          if (bonus.stats.whirlwindcost)
            this.whirlwindcost = bonus.stats.whirlwindcost;
          if (bonus.stats.deathwishcd)
            this.deathwishcd = bonus.stats.deathwishcd;
          if (bonus.stats.bleedbonus) this.bleedbonus = bonus.stats.bleedbonus;
          if (bonus.stats.shockwavecd)
            this.shockwavecd = bonus.stats.shockwavecd;
        }
      }
    }
  }
  addBuffs() {
    ...
    for (let buff of buffs) {
      if (buff.active) {
        ...
        if (buff.group == "dragonbreath") this.dragonbreath = true;
        if (buff.bleedmod) this.bleedmod *= buff.bleedmod;
        if (buff.armor)
          this.target.basearmorbuffed -=
            buff.armor +
            ((buff.name == "Expose Armor" || buff.name == "Sebacious Poison") &&
            this.improvedexposed
              ? buff.armor * 0.5
              : 0);
        if (buff.armorperlevel)
          this.target.basearmorbuffed -= buff.armorperlevel * this.level;
        if (buff.name == "Faerie Fire") this.faeriefire = true;
        if (buff.dmgshield && this.shield)
          this.base.dmgmod *= 1 + buff.dmgshield / 100 || 1;
        if (buff.voodoofrenzy) this.auras.voodoofrenzy = new VoodooFrenzy(this);
        if (buff.stance) {
        ...
        if (buff.resist) {
          let impmotw =
            buff.name == "Mark of the Wild" &&
            buffs.filter((s) => s.motwmod && s.active)[0];
          if (
            buff.name == "Mark of the Wild" &&
            buffs.filter((s) => s.mrp && s.active)[0]
          )
            continue;
          if (
            (buff.name == "Mark of the Wild" || buff.mrp) &&
            buffs.filter((s) => s.fra && s.active)[0]
          )
            continue;
          this.base.resist.fire += ~~(
            buff.resist.fire * (impmotw ? impmotw.motwmod : 1) || 0
          );
          this.base.resist.frost += ~~(
            buff.resist.frost * (impmotw ? impmotw.motwmod : 1) || 0
          );
          this.base.resist.nature += ~~(
            buff.resist.nature * (impmotw ? impmotw.motwmod : 1) || 0
          );
          this.base.resist.shadow += ~~(
            buff.resist.shadow * (impmotw ? impmotw.motwmod : 1) || 0
          );
        }
      }
    }
  ...
  addSpells(testItem: number | undefined) {
    ...
    for (let spell of spells) {
      if (
        spell.item &&
        this.items.includes(spell.id) &&
        spell.id == testItem &&
        spell.id == testItem &&
        !spell.timetoendactive &&
        !spell.timetostartactive
      ) {
        // Blademasters Fury
      ...
      if (
        ...
        (spell.item &&
          this.items.includes(spell.id) &&
          (spell.timetoendactive || spell.timetostartactive))
      ) {
  ...
  sortSpells() {
    ...
    for (let i = 10; i > 0; i--) {
      for (let s in this.spells) {
        if (this.spells[s].priority == i)
          this.normalspells.push(this.spells[s]);
        if (this.spells[s].expriority == i)
          this.executespells.push(this.spells[s]);
      }
      for (let a in this.auras) {
        if (this.auras[a].priority == i) this.normalspells.push(this.auras[a]);
        if (this.auras[a].expriority == i)
          this.executespells.push(this.auras[a]);
      }
    }
  ...
  setSkills() {
    this.base.skill_0 += this.mh.twohand
      ? this.base.skill_20
      : this.base.skill_10;
    this.base.skill_1 += this.mh.twohand
      ? this.base.skill_21
      : this.base.skill_11;
    this.base.skill_3 += this.mh.twohand
      ? this.base.skill_23
      : this.base.skill_13;
  }
  reset(rage) {
    ...
    if (this.trinketproc1 && this.trinketproc1.usestep)
      this.trinketproc1.usestep = 0;
    if (this.trinketproc2 && this.trinketproc2.usestep)
      this.trinketproc2.usestep = 0;
  ...
  getAgiPerCrit(level: number) {
    let table = [
      ...
      0.1282, 0.1282, 0.125, 0.119, 0.1163, 0.1111, 0.1087, 0.1064, 0.102, 0.1,
      0.0962, 0.0943, 0.0926, 0.0893, 0.0877, 0.0847, 0.0833, 0.082, 0.0794,
      0.0781, 0.0758, 0.0735, 0.0725, 0.0704, 0.0694, 0.0676, 0.0667, 0.0649,
  ...
  getTargetSpellBinaryResist() {
    return parseInt(
      10000 -
        (10000 - this.target.misschance) *
          (1 - (this.target.resistance * 0.15) / (this.level * 100)),
    );
  }
  updateStrength() {
    ...
    for (let name in this.auras) {
      if (this.auras[name].timer) {
        ...
        if (this.auras[name].mult_stats.baseapmod)
          this.stats.baseapmod *=
            1 + this.auras[name].mult_stats.baseapmod / 100;
      }
    }
    this.stats.str = ~~(this.stats.str * this.stats.strmod);
    this.stats.ap += this.stats.str * 2;
  ...
  updateHaste() {
    ...
    if (
      this.auras.blisteringragehammer &&
      this.auras.blisteringragehammer.timer
    )
      this.stats.haste *=
        1 + this.auras.blisteringragehammer.mult_stats.haste / 100;
    if (
      this.auras.gyromaticacceleration &&
      this.auras.gyromaticacceleration.timer
    )
      this.stats.haste *=
        1 + this.auras.gyromaticacceleration.mult_stats.haste / 100;
  ...
  updateBonusDmg() {
    ...
    if (this.oh) this.oh.bonusdmg = this.oh.basebonusdmg;
  }
  updateArmorReduction() {
    ...
    if (this.auras.rivenspike && this.auras.rivenspike.timer)
      this.target.armor = Math.max(
        this.target.armor -
          this.auras.rivenspike.stacks * this.auras.rivenspike.armor,
        0,
      );
    if (this.auras.vibroblade && this.auras.vibroblade.timer)
      this.target.armor = Math.max(
        this.target.armor - this.auras.vibroblade.armor,
        0,
      );
    if (this.auras.ultrasonic && this.auras.ultrasonic.timer)
      this.target.armor = Math.max(
        this.target.armor - this.auras.ultrasonic.armor,
        0,
      );
    if (this.auras.cleavearmor && this.auras.cleavearmor.timer)
      this.target.armor = Math.max(
        this.target.armor - this.auras.cleavearmor.armor,
        0,
      );
  ...
  addRage(dmg, result, weapon, spell) {
    ...
    if (this.extracritrage && result == RESULT.CRIT)
      this.rage += this.extracritrage;
  ...
  stepragetimer(a) {
    if (this.ragetimer <= a) {
      ...
      /* start-log */ if (this.logging)
        this.log("10 rage gained"); /* end-log */
      return true;
    } else {
      this.ragetimer -= a;
      return false;
  ...
  stepauras(nobleeds) {
    ...
    if (
      this.auras.mildlyirradiated &&
      this.auras.mildlyirradiated.firstuse &&
      this.auras.mildlyirradiated.timer
    )
      this.auras.mildlyirradiated.step();
    if (
    ...
    if (
      this.auras.voidmadness &&
      this.auras.voidmadness.firstuse &&
      this.auras.voidmadness.timer
    )
      this.auras.voidmadness.step();
    if (
      this.auras.gyromaticacceleration &&
      this.auras.gyromaticacceleration.firstuse &&
      this.auras.gyromaticacceleration.timer
    )
      this.auras.gyromaticacceleration.step();
    if (
      this.auras.gneurological &&
      this.auras.gneurological.firstuse &&
      this.auras.gneurological.timer
    )
      this.auras.gneurological.step();
    if (this.auras.coinflip && this.auras.coinflip.timer)
      this.auras.coinflip.step();
    ...
    if (
      this.auras.roarguardian &&
      this.auras.roarguardian.firstuse &&
      this.auras.roarguardian.timer
    )
      this.auras.roarguardian.step();
    if (
      this.auras.pummeler &&
    ...
    if (
      this.auras.swarmguard &&
      this.auras.swarmguard.firstuse &&
      this.auras.swarmguard.timer
    )
      this.auras.swarmguard.step();
    if (
      this.auras.zandalarian &&
      this.auras.zandalarian.firstuse &&
      this.auras.zandalarian.timer
    )
      this.auras.zandalarian.step();
    if (
      this.auras.relentlessstrength &&
      this.auras.relentlessstrength.firstuse &&
      this.auras.relentlessstrength.timer
    )
      this.auras.relentlessstrength.step();
    if (this.auras.rampage && this.auras.rampage.timer)
      this.auras.rampage.step();
    if (this.auras.wreckingcrew && this.auras.wreckingcrew.timer)
      this.auras.wreckingcrew.step();
    if (this.auras.freshmeat && this.auras.freshmeat.timer)
      this.auras.freshmeat.step();
    if (this.auras.suddendeath && this.auras.suddendeath.timer)
      this.auras.suddendeath.step();
    if (this.auras.voodoofrenzy && this.auras.voodoofrenzy.timer)
      this.auras.voodoofrenzy.step();
    if (this.auras.battleshout && this.auras.battleshout.timer)
      this.auras.battleshout.step();
    if (this.auras.echoeszerk && this.auras.echoeszerk.timer)
      this.auras.echoeszerk.step();
    if (this.auras.echoesbattle && this.auras.echoesbattle.timer)
      this.auras.echoesbattle.step();
    if (this.auras.echoesdef && this.auras.echoesdef.timer)
      this.auras.echoesdef.step();
    if (this.auras.echoesglad && this.auras.echoesglad.timer)
      this.auras.echoesglad.step();
    if (this.auras.battleforecast && this.auras.battleforecast.timer)
      this.auras.battleforecast.step();
    if (this.auras.zerkforecast && this.auras.zerkforecast.timer)
      this.auras.zerkforecast.step();
    if (this.auras.defforecast && this.auras.defforecast.timer)
      this.auras.defforecast.step();
    if (this.auras.gladforecast && this.auras.gladforecast.timer)
      this.auras.gladforecast.step();
    if (this.auras.defendersresolve && this.auras.defendersresolve.timer)
      this.auras.defendersresolve.step();
    if (this.auras.singleminded && this.auras.singleminded.timer)
      this.auras.singleminded.step();
    if (this.auras.demontaintedblood && this.auras.demontaintedblood.timer)
      this.auras.demontaintedblood.step();
    if (this.auras.wrathwray && this.auras.wrathwray.timer)
      this.auras.wrathwray.step();
    if (this.auras.moonstalkerfury && this.auras.moonstalkerfury.timer)
      this.auras.moonstalkerfury.step();
    if (this.auras.jujuflurry && this.auras.jujuflurry.timer)
      this.auras.jujuflurry.step();
    if (this.auras.grilekguard && this.auras.grilekguard.timer)
      this.auras.grilekguard.step();
    if (this.auras.obsidianhaste && this.auras.obsidianhaste.timer)
      this.auras.obsidianhaste.step();
    if (this.auras.obsidianstrength && this.auras.obsidianstrength.timer)
      this.auras.obsidianstrength.step();
    ...
    if (
      this.attackproc1 &&
      this.attackproc1.spell &&
      this.attackproc1.spell.timer
    )
      this.attackproc1.spell.step();
    if (
      this.attackproc2 &&
      this.attackproc2.spell &&
      this.attackproc2.spell.timer
    )
      this.attackproc2.spell.step();

    if (!nobleeds && this.auras.deepwounds && this.auras.deepwounds.timer)
      this.auras.deepwounds.step();
    if (!nobleeds && this.auras.rend && this.auras.rend.timer)
      this.auras.rend.step();
    if (this.auras.berserkerrage && this.auras.berserkerrage.timer)
      this.auras.berserkerrage.step();
    if (this.auras.consumedrage && this.auras.consumedrage.timer)
      this.auras.consumedrage.step();
    if (this.auras.weaponbleedmh && this.auras.weaponbleedmh.timer)
      this.auras.weaponbleedmh.step();
    if (this.auras.weaponbleedoh && this.auras.weaponbleedoh.timer)
      this.auras.weaponbleedoh.step();

    if (!nobleeds && this.adjacent) {
      if (this.auras.deepwounds2 && this.auras.deepwounds2.timer)
        this.auras.deepwounds2.step();
      if (this.auras.deepwounds3 && this.auras.deepwounds3.timer)
        this.auras.deepwounds3.step();
      if (this.auras.deepwounds4 && this.auras.deepwounds4.timer)
        this.auras.deepwounds4.step();
    }
  }
  endauras() {
    if (this.mh.proc1 && this.mh.proc1.spell && this.mh.proc1.spell.timer)
      this.mh.proc1.spell.end();
    if (this.mh.proc2 && this.mh.proc2.spell && this.mh.proc2.spell.timer)
      this.mh.proc2.spell.end();
    if (
      this.oh &&
      this.oh.proc1 &&
      this.oh.proc1.spell &&
    ...
    if (
      ...
      this.auras.mightyragepotion.end();
    if (
      this.auras.mildlyirradiated &&
      this.auras.mildlyirradiated.firstuse &&
      this.auras.mildlyirradiated.timer
    )
      this.auras.mildlyirradiated.end();
    if (
      this.auras.recklessness &&
      this.auras.recklessness.firstuse &&
      this.auras.recklessness.timer
    )
      this.auras.recklessness.end();
    if (
      this.auras.deathwish &&
      this.auras.deathwish.firstuse &&
      this.auras.deathwish.timer
    )
      this.auras.deathwish.end();
    if (
      this.auras.cloudkeeper &&
      this.auras.cloudkeeper.firstuse &&
      this.auras.cloudkeeper.timer
    )
      this.auras.cloudkeeper.end();
    if (
      this.auras.voidmadness &&
      this.auras.voidmadness.firstuse &&
      this.auras.voidmadness.timer
    )
      this.auras.voidmadness.end();
    if (
      this.auras.gyromaticacceleration &&
      this.auras.gyromaticacceleration.firstuse &&
      this.auras.gyromaticacceleration.timer
    )
      this.auras.gyromaticacceleration.end();
    if (
      this.auras.gneurological &&
      this.auras.gneurological.firstuse &&
      this.auras.gneurological.timer
    )
      this.auras.gneurological.end();
    if (this.auras.coinflip && this.auras.coinflip.timer)
      this.auras.coinflip.end();
    if (this.auras.flask && this.auras.flask.firstuse && this.auras.flask.timer)
      this.auras.flask.end();
    if (
      this.auras.bloodfury &&
      this.auras.bloodfury.firstuse &&
      this.auras.bloodfury.timer
    )
      this.auras.bloodfury.end();
    if (
      this.auras.berserking &&
      this.auras.berserking.firstuse &&
      this.auras.berserking.timer
    )
      this.auras.berserking.end();
    if (
      this.auras.slayer &&
      this.auras.slayer.firstuse &&
      this.auras.slayer.timer
    )
      this.auras.slayer.end();
    if (
      this.auras.spider &&
      this.auras.spider.firstuse &&
      this.auras.spider.timer
    )
      this.auras.spider.end();
    if (
      this.auras.gabbar &&
      this.auras.gabbar.firstuse &&
      this.auras.gabbar.timer
    )
      this.auras.gabbar.end();
    if (
      this.auras.earthstrike &&
      this.auras.earthstrike.firstuse &&
      this.auras.earthstrike.timer
    )
      this.auras.earthstrike.end();
    if (
      this.auras.roarguardian &&
      this.auras.roarguardian.firstuse &&
      this.auras.roarguardian.timer
    )
      this.auras.roarguardian.end();
    if (
      this.auras.pummeler &&
      this.auras.pummeler.firstuse &&
      this.auras.pummeler.timer
    )
      this.auras.pummeler.end();
    if (
      this.auras.swarmguard &&
      this.auras.swarmguard.firstuse &&
      this.auras.swarmguard.timer
    )
      this.auras.swarmguard.end();
    if (
      this.auras.zandalarian &&
      this.auras.zandalarian.firstuse &&
      this.auras.zandalarian.timer
    )
      this.auras.zandalarian.end();
    if (
      this.auras.relentlessstrength &&
      this.auras.relentlessstrength.firstuse &&
      this.auras.relentlessstrength.timer
    )
      this.auras.relentlessstrength.end();
    if (this.auras.rampage && this.auras.rampage.timer)
      this.auras.rampage.end();
    if (this.auras.wreckingcrew && this.auras.wreckingcrew.timer)
      this.auras.wreckingcrew.end();
    if (this.auras.freshmeat && this.auras.freshmeat.timer)
      this.auras.freshmeat.end();
    if (this.auras.suddendeath && this.auras.suddendeath.timer)
      this.auras.suddendeath.end();
    if (this.auras.voodoofrenzy && this.auras.voodoofrenzy.timer)
      this.auras.voodoofrenzy.end();
    if (this.auras.battleshout && this.auras.battleshout.timer)
      this.auras.battleshout.end();
    if (this.auras.echoeszerk && this.auras.echoeszerk.timer)
      this.auras.echoeszerk.end();
    if (this.auras.echoesbattle && this.auras.echoesbattle.timer)
      this.auras.echoesbattle.end();
    if (this.auras.echoesdef && this.auras.echoesdef.timer)
      this.auras.echoesdef.end();
    if (this.auras.echoesglad && this.auras.echoesglad.timer)
      this.auras.echoesglad.end();
    if (this.auras.battleforecast && this.auras.battleforecast.timer)
      this.auras.battleforecast.end();
    if (this.auras.zerkforecast && this.auras.zerkforecast.timer)
      this.auras.zerkforecast.end();
    if (this.auras.defforecast && this.auras.defforecast.timer)
      this.auras.defforecast.end();
    if (this.auras.gladforecast && this.auras.gladforecast.timer)
      this.auras.gladforecast.end();
    if (this.auras.defendersresolve && this.auras.defendersresolve.timer)
      this.auras.defendersresolve.end();
    if (this.auras.singleminded && this.auras.singleminded.timer)
      this.auras.singleminded.end();
    if (this.auras.moonstalkerfury && this.auras.moonstalkerfury.timer)
      this.auras.moonstalkerfury.end();
    if (this.auras.demontaintedblood && this.auras.demontaintedblood.timer)
      this.auras.demontaintedblood.end();
    if (this.auras.wrathwray && this.auras.wrathwray.timer)
      this.auras.wrathwray.end();
    if (this.auras.jujuflurry && this.auras.jujuflurry.timer)
      this.auras.jujuflurry.end();
    if (this.auras.grilekguard && this.auras.grilekguard.timer)
      this.auras.grilekguard.end();
    if (this.auras.obsidianhaste && this.auras.obsidianhaste.timer)
      this.auras.obsidianhaste.end();
    if (this.auras.obsidianstrength && this.auras.obsidianstrength.timer)
      this.auras.obsidianstrength.end();
    ...
    if (
      ...
      this.trinketproc1.spell.end();
    if (
      this.trinketproc2 &&
      this.trinketproc2.spell &&
      this.trinketproc2.spell.timer
    )
      this.trinketproc2.spell.end();
    if (
      this.attackproc1 &&
      this.attackproc1.spell &&
      this.attackproc1.spell.timer
    )
      this.attackproc1.spell.end();
    if (
      this.attackproc2 &&
      this.attackproc2.spell &&
      this.attackproc2.spell.timer
    )
      this.attackproc2.spell.end();

    if (this.auras.flurry && this.auras.flurry.timer) this.auras.flurry.end();
    if (this.auras.deepwounds && this.auras.deepwounds.timer)
      this.auras.deepwounds.end();
    if (this.auras.deepwounds2 && this.auras.deepwounds2.timer)
      this.auras.deepwounds2.end();
    if (this.auras.deepwounds3 && this.auras.deepwounds3.timer)
      this.auras.deepwounds3.end();
    if (this.auras.deepwounds4 && this.auras.deepwounds4.timer)
      this.auras.deepwounds4.end();
    if (this.auras.rend && this.auras.rend.timer) this.auras.rend.end();
    if (this.auras.berserkerrage && this.auras.berserkerrage.timer)
      this.auras.berserkerrage.end();
    if (this.auras.consumedrage && this.auras.consumedrage.timer)
      this.auras.consumedrage.end();
    if (this.auras.weaponbleedmh && this.auras.weaponbleedmh.timer)
      this.auras.weaponbleedmh.end();
    if (this.auras.weaponbleedoh && this.auras.weaponbleedoh.timer)
      this.auras.weaponbleedoh.end();
  }
  rollweapon(weapon) {
    let tmp = 0;
    let roll = rng10k();
  ...
  rollmagicspell(spell) {
    ...
    if (rng10k() < miss) return RESULT.MISS;
  ...
  attackmh(weapon, adjacent, damageSoFar) {
    ...
    if (this.nextswinghs) {
      this.nextswinghs = false;
      if (
        this.spells.heroicstrike &&
        this.spells.heroicstrike.cost <= this.rage
      ) {
        ...
        /* start-log */ if (this.logging)
          this.log(`Heroic Strike auto canceled`); /* end-log */
      }
    } else {
      result = this.rollweapon(weapon);
    }
    if (spell && this.spells.ragingblow) this.spells.ragingblow.reduce(spell);
    ...
    if (result == RESULT.CRIT) {
      ...
      let critmod =
        1 +
        1 *
          (1 + (spell ? this.talents.abilitiescrit : 0)) *
          (1 + this.critdmgbonus * 2);
      dmg *= critmod;
    ...
    /* start-log */ if (this.logging)
      this.log(
        `${spell ? spell.name + " for" : "Main hand attack for"} ${~~done} (${
  ...
  attackoh(weapon) {
    ...
    /* start-log */ if (this.logging)
      this.log(
        `Off hand attack for ${done + procdmg} (${
          Object.keys(RESULT)[result]
        })${this.nextswinghs ? " (HS queued)" : ""}`,
      ); /* end-log */
    return done + procdmg;
  }
  cast(spell, delayedheroic, adjacent, damageSoFar) {
    ...
    if (spell.useonly) {
      /* start-log */ if (this.logging)
        this.log(`${spell.name} used`); /* end-log */
      return 0;
    }
    ...
    if (result == RESULT.MISS) {
      ...
      if (spell.defenseType == DEFENSETYPE.MAGIC)
        critmod =
          1 +
          0.5 * (1 + this.talents.abilitiescrit) * (1 + this.critdmgbonus * 3);
      else
        critmod =
          1 +
          1 * (1 + this.talents.abilitiescrit) * (1 + this.critdmgbonus * 2);
    ...
    /* start-log */ if (this.logging)
      this.log(
        `${spell.name} for ${~~done} (${Object.keys(RESULT)[result]})${
          adjacent ? " (Adjacent)" : ""
        }.`,
      ); /* end-log */
    return done + procdmg;
  ...
  castoh(spell, adjacent, damageSoFar) {
    ...
    if (result == RESULT.MISS) {
      ...
      let critmod =
        1 + 1 * (1 + this.talents.abilitiescrit) * (1 + this.critdmgbonus * 2);
      dmg *= critmod;
      this.proccrit(false, adjacent, spell);
    }
    ...
    /* start-log */ if (this.logging)
      this.log(
        `${spell.name} (OH) for ${~~done} (${Object.keys(RESULT)[result]})${
          adjacent ? " (Adjacent)" : ""
        }.`,
      ); /* end-log */
    return done + procdmg;
  ...
  procattack(spell, weapon, result, adjacent, damageSoFar) {
    ...
    if (result != RESULT.MISS && result != RESULT.DODGE) {
      ...
      if (spell instanceof Slam && this.slammainreset) {
        ...
        /* start-log */ if (this.logging)
          this.log(`T2 Slam reset`); /* end-log */
      }
      if (
        weapon.proc1 &&
        !weapon.proc1.extra &&
        rng10k() < weapon.proc1.chance &&
        !(weapon.proc1.gcd && this.timer && this.timer < 1500)
      ) {
        if (weapon.proc1.spell) weapon.proc1.spell.use();
        if (weapon.proc1.magicdmg)
          procdmg +=
            weapon.proc1.chance == 10000
              ? weapon.proc1.magicdmg
              : this.magicproc(weapon.proc1);
        ...
        /* start-log */ if (this.logging)
          this.log(
            `${weapon.name} proc ${procdmg ? "for " + ~~procdmg : ""}`,
          ); /* end-log */
      }
      ...
      if (
        this.trinketproc1 &&
        !this.trinketproc1.extra &&
        rng10k() < this.trinketproc1.chance
      ) {
        if (this.trinketproc1.magicdmg)
          procdmg += this.magicproc(this.trinketproc1);
        if (this.trinketproc1.spell) this.trinketproc1.spell.use();
        /* start-log */ if (this.logging)
          this.log(`Trinket 1 proc`); /* end-log */
      }
      if (
        this.trinketproc1 &&
        this.trinketproc1.extra &&
        ...
        if (
          !this.trinketproc1.cooldown ||
          !this.trinketproc1.usestep ||
          step > this.trinketproc1.usestep
        ) {
          if (this.trinketproc1.cooldown)
            this.trinketproc1.usestep = step + this.trinketproc1.cooldown;
      ...
      if (
        this.trinketproc2 &&
        !this.trinketproc2.extra &&
        rng10k() < this.trinketproc2.chance
      ) {
        if (this.trinketproc2.magicdmg)
          procdmg += this.magicproc(this.trinketproc2);
        if (this.trinketproc2.spell) this.trinketproc2.spell.use();
        /* start-log */ if (this.logging)
          this.log(`Trinket 2 proc`); /* end-log */
      }
      if (
        this.trinketproc2 &&
        this.trinketproc2.extra &&
        ...
        if (
          !this.trinketproc2.cooldown ||
          !this.trinketproc2.usestep ||
          step > this.trinketproc2.usestep
        ) {
          if (this.trinketproc2.cooldown)
            this.trinketproc2.usestep = step + this.trinketproc2.cooldown;
      ...
      if (this.attackproc1 && rng10k() < this.attackproc1.chance) {
        if (this.attackproc1.magicdmg) {
          procdmg +=
            this.attackproc1.chance == 10000
              ? this.attackproc1.magicdmg
              : this.magicproc(this.attackproc1);
          /* start-log */ if (this.logging)
            this.log(`Attack proc for ${procdmg}`); /* end-log */
        }
      ...
      if (this.attackproc2 && rng10k() < this.attackproc2.chance) {
        if (this.attackproc2.magicdmg) {
          procdmg +=
            this.attackproc2.chance == 10000
              ? this.attackproc2.magicdmg
              : this.magicproc(this.attackproc2);
          /* start-log */ if (this.logging)
            this.log(`Attack proc for ${procdmg}`); /* end-log */
        }
      ...
      if (
        this.talents.swordproc &&
        weapon.type == WEAPONTYPE.SWORD &&
        !damageSoFar &&
        this.swordspecstep != step &&
        rng10k() < this.talents.swordproc * 100
      ) {
        this.swordspecstep = step;
        if (spell) this.extraattacks++;
        else extras++;
        /* start-log */ if (this.logging)
          this.log(`Sword talent proc`); /* end-log */
      }
      ...
      if (
        ...
        /* start-log */ if (this.logging)
          this.log(`Wailing set extra attack proc`); /* end-log */
      }
      ...
      if (
        ...
        /* start-log */ if (this.logging)
          this.log(`Hakkari set extra attack proc`); /* end-log */
      }
      // Timeworn extra
      if (
        this.timeworn &&
        ...
        rng10k() < this.timeworn * 100
      ) {
        this.timewornstep = step;
        if (spell) this.extraattacks++;
        else extras++;
        /* start-log */ if (this.logging)
          this.log(`Timeworn proc`); /* end-log */
      }
      // Obsidian Champion
      if (
        weapon.id == 233490 &&
        rng10k() < weapon.proc1.chance &&
        !(this.timer && this.timer < 1500)
      ) {
        this.auras.obsidianstrength.use();
        /* start-log */ if (this.logging)
          this.log(`${weapon.name} Strength proc`); /* end-log */
      }
      if (
        weapon.id == 233490 &&
        rng10k() < weapon.proc1.chance &&
        ...
        /* start-log */ if (this.logging)
          this.log(`${weapon.name} Haste proc`); /* end-log */
      }
      // Blood Surge
      if (
        this.bloodsurge &&
        ...
        this.freeslam = true;
        /* start-log */ if (this.logging)
          this.log(`Blood Surge proc`); /* end-log */
      }
      // Sword and Board
      if (
        this.swordboard &&
        ...
        /* start-log */ if (this.logging)
          this.log(`Sword and Board proc`); /* end-log */
      }
      // Voodoo Frenzy
      ...
      if (
        this.auras.swarmguard &&
        this.auras.swarmguard.timer &&
        rng10k() < this.auras.swarmguard.chance
      ) {
  ...
  phantomproc(weapon) {
    ...
    if (weapon.proc2 && rng10k() < weapon.proc2.chance) {
      ...
      /* start-log */ if (this.logging)
        this.log(`${weapon.name} proc`); /* end-log */
    }
    return dmg;
  }
  magicproc(proc) {
    ...
    if (rng10k() < this.stats.spellcrit * 100)
      mod *= 1 + 0.5 * (1 + this.critdmgbonus * 3);
  ...
  log(msg) {
    ...
    else if (msg.indexOf("applied") > 1 || msg.indexOf("removed") > -1)
      color = "#17A8B6";
    console.log(
      `%c ${(step / 1000).toFixed(3).padStart(6, " ")} | ${this.rage
        .toFixed(2)
        .padStart(6, " ")} | ${msg}`,
      `color: ${color}`,
    );
  ...
  switch(stance) {
    ...
    if (this.auras[this.stance + "forecast"])
      this.auras[this.stance + "forecast"].use();

    this.ragemod =
      (this.base.ragemod || 1) *
      (this.stance == "glad" && !this.target.speed ? 1.5 : 1);
  ...
  isValidStance(stance, isRend) {
    return (
      ...
      (stance == "zerk" &&
        this.auras.echoeszerk &&
        this.auras.echoeszerk.timer) ||
      (stance == "battle" &&
        this.auras.echoesbattle &&
        this.auras.echoesbattle.timer) ||
  ...
  isEnraged() {
    return (
      (this.auras.wreckingcrew && this.auras.wreckingcrew.timer) ||

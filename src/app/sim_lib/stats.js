...
SIM.STATS = {
  ...
  buildData: function (sim) {
    ...
    for (let name in sim.player.auras) {
      ...
      data.push(
        Math.min((aura.uptime / sim.totalduration / 10).toFixed(2), 100),
      );
    ...
    if (sim.player.auras.deepwounds && sim.player.auras.deepwounds.totaldmg) {
      ...
      data.push(
        (sim.player.auras.deepwounds.totaldmg / sim.totalduration).toFixed(2),
      );
      colors.push(view.colors[counter % view.colors.length]);
    }
    if (sim.player.auras.deepwounds2 && sim.player.auras.deepwounds2.totaldmg) {
      ...
      data.push(
        (sim.player.auras.deepwounds2.totaldmg / sim.totalduration).toFixed(2),
      );
      colors.push(view.colors[counter % view.colors.length]);
    }
    if (sim.player.auras.deepwounds3 && sim.player.auras.deepwounds3.totaldmg) {
      ...
      data.push(
        (sim.player.auras.deepwounds3.totaldmg / sim.totalduration).toFixed(2),
      );
      colors.push(view.colors[counter % view.colors.length]);
    }
    if (sim.player.auras.deepwounds4 && sim.player.auras.deepwounds4.totaldmg) {
      ...
      data.push(
        (sim.player.auras.deepwounds4.totaldmg / sim.totalduration).toFixed(2),
      );
      colors.push(view.colors[counter % view.colors.length]);
    }
    ...
    if (sim.player.auras.rend && sim.player.auras.rend.totaldmg) {
      ...
      data.push(
        (sim.player.auras.rend.totaldmg / sim.totalduration).toFixed(2),
      );
      colors.push(view.colors[counter % view.colors.length]);
    }
    ...
    // weapon bleed
    if (
      sim.player.auras.weaponbleedmh &&
      sim.player.auras.weaponbleedmh.totaldmg
    ) {
      view.dmgdata.labels.push(sim.player.auras.weaponbleedmh.name);
      data.push(
        (sim.player.auras.weaponbleedmh.totaldmg / sim.totalduration).toFixed(
          2,
        ),
      );
      colors.push(view.colors[counter % view.colors.length]);
    ...
    if (
      sim.player.auras.weaponbleedoh &&
      sim.player.auras.weaponbleedoh.totaldmg
    ) {
      view.dmgdata.labels.push(sim.player.auras.weaponbleedoh.name);
      data.push(
        (sim.player.auras.weaponbleedoh.totaldmg / sim.totalduration).toFixed(
          2,
        ),
      );
      colors.push(view.colors[counter % view.colors.length]);
    ...
    view.dmgchart = new Chart(view.dmg, {
      ...
      options: {
        ...
        tooltips: {
          callbacks: {
            label: (item, obj) =>
              ` ${obj.labels[item.index]}: ${
                obj.datasets[0].data[item.index]
              } DPS`,
          },
        },
      },
  ...
  buildTable: function (sim) {
    ...
    html += `<tr><td>Main Hand</td><td>${((data[0] / total) * 100).toFixed(
      2,
    )}</td><td>${((data[3] / total) * 100).toFixed(2)}</td><td>${(
      (data[1] / total) *
      100
    ).toFixed(2)}</td><td>${((data[2] / total) * 100).toFixed(2)}</td><td>${(
      (data[4] / total) *
      100
    ).toFixed(2)}</td><td>${(total / i).toFixed(
      2,
    )}</td><td></td><td>${dps}</td></tr>`;

    if (sim.player.oh) {
      data = sim.player.oh.data;
      total = data.reduce((a, b) => a + b, 0);
      dps = (sim.player.oh.totaldmg / sim.totalduration).toFixed(2);
      html += `<tr><td>Off Hand</td><td>${((data[0] / total) * 100).toFixed(
        2,
      )}</td><td>${((data[3] / total) * 100).toFixed(2)}</td><td>${(
        (data[1] / total) *
        100
    ...
    for (let name in sim.player.spells) {
      ...
      let total = data.reduce((a, b) => a + b, 0);
      if (!total) continue;
      let dps = (sim.player.spells[name].totaldmg / sim.totalduration).toFixed(
        2,
      );
      let dpr = (
      ...
      if (name == "execute")
        dpr = (
          ...
          i /
          (sim.player.spells[name].cost * (total / i) +
            sim.player.spells[name].totalusedrage / i)
        ).toFixed(2);
      html += `<tr><td>${n}</td><td>${((data[0] / total) * 100).toFixed(
        2,
      )}</td><td>${((data[3] / total) * 100).toFixed(2)}</td><td>${(
        (data[1] / total) *
        100
    ...
    if (sim.player.auras.rend) {
      ...
      let dpr = (
        totaldmg /
        i /
        (sim.player.auras.rend.cost * (total / i))
      ).toFixed(2);
      html += `<tr><td>${
        sim.player.auras.rend.name
      }</td><td></td><td></td><td>${((data[1] / total) * 100).toFixed(
        2,
      )}</td><td>${((data[2] / total) * 100).toFixed(2)}</td><td></td><td>${(
        total / i
      ).toFixed(2)}</td><td>${dpr}</td><td>${dps}</td></tr>`;
    }
